...
function toggleExpand(btn) {
    ...
    // If it's the chart container we trigger resize 
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function generateTable(container) {
        ...
        // 3) Fetch Data
        // Use the NEW PHP file for the Frequency
        fetch(`get_disruption_frequency.php?${params.toString()}`)
            .then(r => {
                if (!r.ok) {
            ...
            .then(data => {
                ...
                // 4) Render Table
                // Re-adding button is safer since we clear innerHTML
                ...
                const headers = ["Company", "Disruption Frequency (per Year)", "Ranking"];
                ...
                data.forEach((row, index) => {
                    ...
                    // Use the new field disruptionFrequency
                    tdVal.textContent = parseFloat(row.disruptionFrequency).toFixed(2);
    ...
    function renderChart(data, limit = 'All') {
        const graphContainer = document.querySelector('.graph-container');

        // Ensure Expand Button dose Exists
        ...
        let chartData = [...data].sort((a, b) => parseFloat(b.disruptionFrequency) - parseFloat(a.disruptionFrequency));
        ...
        const values = chartData.map(d => parseFloat(d.disruptionFrequency));
        ...
        myChart = new Chart(ctx, {
            ...
            data: {
                labels: labels,
                datasets: [{
                    label: 'Disruption Frequency (per Year)',
                    data: values,
            ...
            options: {
                ...
                scales: {
                    y: {
                        ...
                        title: {
                            display: true,
                            text: 'Disruption Frequency (per Year)'
                        }
                    },
                    x: {
                        title: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        callbacks: {
                            label: (item) => `Frequency: ${item.raw} / year`
                        }
                    }
                }
            }
    ...
    dateFilterCheckbox.addEventListener('change', (event) => {
        ...
        if (event.currentTarget.checked) {
            if (!existingDateFilter) {
                ...
                dateContainer.style.cssText = 'margin-top:10px; display:grid; gap:8px; grid-template-columns:auto 1fr; align-items:center; width: 100%;';
