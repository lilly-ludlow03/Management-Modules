...
try {
    ...
    // Build Query
    // Equation: SUM(RecoveryDate - EventDate)
    
    $sql = "
    SELECT 
        c.CompanyName AS company_name,
        COALESCE(SUM(DATEDIFF(e.EventRecoveryDate, e.EventDate)), 0) AS totalDowntime,
    ...
    // Sort by Total Downtime ASC
    $sql .= " ORDER BY totalDowntime ASC";
    ...
    while ($row = $result->fetch_assoc()) {
        $row['totalDowntime'] = (float)$row['totalDowntime'];
        $data[] = $row;
    }
